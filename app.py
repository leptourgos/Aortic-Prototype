...
st.title("🫀 Smart Cut: Phase 17 Omni-Engine (Triple Output)")
...
if uploaded_file is not None:
    if st.button("GENERATE TRIPLE-OUTPUT SUITE", type="primary"):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if stl_path:
                with st.spinner("Processing First-Principles & Multi-Output Suite..."):
                    ...
                    if smooth_passes > 0: mesh = trimesh.smoothing.filter_laplacian(mesh, iterations=smooth_passes)
                    ...
                    step = {"Fast (Low Res)": 50, "Standard": 10, "Ultra": 1}[render_quality]
                    x_ren, y_ren, r_ren = x_flat[::step], y_flat[::step], r[::step]
                    ...
                    if enable_rfid and len(rfid_nodes) > 0:
                        ...
                        for node in target_nodes: ax2.text(x_flat[node]+3, y_flat[node]+3, "RFID", color='#FF00FF', fontsize=6)
